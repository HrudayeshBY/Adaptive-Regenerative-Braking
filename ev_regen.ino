...
const float corneringThreshold = 50.0;         // Threshold for angular velocity (degrees/second)
const float corneringExitThreshold = 35.0;     // Lower threshold to exit cornering
...
const int corneringStableReadingsNeeded = 20;
...
void setup() {
  ...
  pinMode(coastingRelayPin, OUTPUT);
...
void loop() {
  ...
  // Coasting detection
  if (!isCoasting && abs(smoothAccelX) < coastingThreshold) {
  ...
  // Downhill detection
  float tiltAngle = atan2(accelY, accelZ) * 180 / PI;
...
void setRelayState(bool coasting, bool cornering, bool downhill) {
  ...
  digitalWrite(coastingRelayPin, coasting ? HIGH : LOW);
